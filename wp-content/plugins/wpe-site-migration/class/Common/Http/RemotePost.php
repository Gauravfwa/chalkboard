...
class RemotePost extends Http {
	...
	const RESPONSE_STATUS_ERROR_WORDFENCE = 'RESPONSE_STATUS_ERROR_WORDFENCE';
	...
	const RESPONSE_REMOTE_WP_ERROR        = 'RESPONSE_REMOTE_WP_ERROR';
	 ...
	 * @param bool   $return_http_object Return the WP_Http object instead of the response body.
	 *
	 * @return bool|string|Response|WP_Error
	 */
	public function post( $url, $data, $scope, $args = [], $expecting_serial = false, $return_http_object = false ) {
	...
	public function handle_remote_post_responses( $response, $url, $scope, $expecting_serial, $state_data = array() ) {
		...
		// If error looks like a WP_Error.
		if ( isset( $decoded_body['data'] ) && Util::array_is_wp_error( $decoded_body['data'] ) ) {
			$this->remote_error = $decoded_body['data'];

			return self::RESPONSE_REMOTE_WP_ERROR;
		}
	...
	/**
	 * Handle the response from a POST request and returns a WP_Error object
	 * with relevant code, message and data.
	 *
	 * Returns false if the $code is not recognized.
	 *
	 * @param string         $code             The HTTP response code.
	 * @param WP_Error|array $response         The response data/object.
	 * @param string         $url              The URL the request was made to.
	 * @param array          $data             Data that was sent in the body of the request.
	 * @param string         $scope            A string to be used in error messages defining the function that initiated the remote post.
	 * @param array          $args             Arguments from the original request.
	 * @param boolean        $expecting_serial Flag that was set specifying if we should verify that the response is a serialized string.
	 *
		...
		switch ( $code ) {
			...
			case self::RESPONSE_401_ERROR:
				return new WP_Error(
					...
					Util::format_additional_error_data( Util::format_http_error_data( $response ), [ 'url' => $url ] )
				);
			...
			case self::RESPONSE_STATUS_ERROR_WORDFENCE:
				return new WP_Error(
					self::RESPONSE_STATUS_ERROR_WORDFENCE,
					Util::get_wordfence_error_message(),
					Util::format_additional_error_data( Util::format_http_error_data( $response ), [ 'url' => $url ] )
				);
			...
			case self::RESPONSE_REMOTE_WP_ERROR:
				return new WP_Error(
					$this->remote_error['code'],
					$this->remote_error['message'],
					$this->remote_error['data']
				);
	...
	protected function handle_http_error_codes( $response, $url, $scope ) {
		switch ( (int) $response['response']['code'] ) {
			...
			case 403 && str_contains( $response['body'], 'wordfence' ):
				return self::RESPONSE_STATUS_ERROR_WORDFENCE;
