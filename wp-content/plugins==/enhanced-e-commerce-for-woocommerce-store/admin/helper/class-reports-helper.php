	...
	class Conversios_Reports_Helper
	{
		...
		public function save_prompt_suggestions()
		{
			...
			if ($this->admin_safe_ajax_call($nonce, 'conversios_nonce')) {
				...
				$suggestions = isset($_POST['data']) ? $_POST['data'] : "";
				...
				if($subscription_id != "" && $domain != "" ) {
					if( !empty($suggestions )) {
						...
						echo json_encode(array('error' => true, 'errors' => esc_html__("Please fill in any suggestions to be submitted.", "enhanced-e-commerce-for-woocommerce-store")));	
					}
				}else{
