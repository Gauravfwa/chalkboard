  ...
  class TVC_Ajax_File extends TVC_Ajax_Calls
  {
    ...
    public function conv_create_ec_row()
    {
      if ($this->safe_ajax_call($_POST['pix_sav_nonce'], 'pix_sav_nonce_val')) {
        ...
        $formdata = array("subscription_id" => $_POST['subscription_id'], "ec_token" => $_POST['ecrandomstring']);
        echo json_encode($customObj->conv_create_ec_row_apicall($formdata));
      }
      die();
    }
    ...
    {
      $customObj = new CustomApi();
      $datadump_main = json_encode(json_decode(stripslashes($_POST['convReportData']), true));
      $is_generate = isset($_POST['is_generate']) && $_POST['is_generate'] == 1 ? 1 : 0;
      $formdata = array(
        "subscription_id" => $_POST['subscription_id'],
        "ec_token" => $_POST['ec_token'],
    ...
    public function conv_save_pixel_data()
    {
      if ($this->safe_ajax_call($_POST['pix_sav_nonce'], 'pix_sav_nonce_val')) {
        ...
        if (in_array("eeconvnotice", $_POST['conv_options_type'])) {
          $this->conv_save_eeconvnotice($_POST);
        }
    ...
    public function conv_save_googleads_data()
    {
      if ($this->safe_ajax_call($_POST['pix_sav_nonce'], 'pix_sav_nonce_val')) {
        ...
        if($conv_options_data['ga_GMC'] == '1') { 
          $data_eeoptions['conv_options_data']['ga_GMC'] = $conv_options_data['ga_GMC'];
        }
        ...
        if($conv_options_data['ga_GMC'] == '1') {
          ...
          $postData = ['merchant_id' => $conv_options_data['merchant_id'], 'account_id' => $conv_options_data['google_merchant_id'], 'adwords_id' => $conv_options_data['google_ads_id']];
