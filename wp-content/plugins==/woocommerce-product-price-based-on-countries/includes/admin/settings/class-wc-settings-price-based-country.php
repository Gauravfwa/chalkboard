...
if ( ! class_exists( 'WC_Settings_Price_Based_Country' ) ) :
	...
	class WC_Settings_Price_Based_Country extends WC_Settings_Page {
		...
		protected function output_license_screen() {
			...
			$license['is_connected'] = WCPBC_License_Settings::is_license_connected();
			...
			if ( empty( $license['key'] ) ) {
				...
				if ( 'expired' === $license['status'] ) {
					...
					$message = sprintf( __( 'The license %1$s %2$sexpired%3$s on %4$s.', 'woocommerce-product-price-based-on-countries' ), '<code>...' . substr( $license['key'], -5 ) . '</code>', '<strong>', '</strong>', $license['expires'] );
				} else {
					// Translators: license key.
					$message = sprintf( __( 'The license %1$s %2$sis not valid or expired%3$s more than 3 months ago.', 'woocommerce-product-price-based-on-countries' ), '<code>...' . substr( $license['key'], -5 ) . '</code>', '<strong>', '</strong>' );
				}
				...
				$license['actions'][] = [
					...
					'icon'         => 'info',
					// Translators: 1,2: HTML tag.
					'message'      => $message . '  ' . sprintf( __( 'To receive updates and support, you need to %1$spurchase%2$s a new license.', 'woocommerce-product-price-based-on-countries' ), '<strong>', '</strong>' ),
					'button_label' => __( 'Purchase', 'woocommerce-product-price-based-on-countries' ),
