...
/**
 *
 * Maintain list of recovered carts.
 *
...
if ( ! class_exists( 'AF_Recovered_Cart_List_Table' ) ) {
	/**
	 * Class for table of abandoned cart.
	 */
	class AF_Recovered_Cart_List_Table extends WP_List_Table {
		...
		public function __construct() {
			...
			parent::__construct(
				array(
					'singular' => __( 'Recovered Cart', 'addify_acr' ), // singular name of the listed records.
					'plural'   => __( 'Recovered Carts', 'addify_acr' ), // plural name of the listed records.
		 ...
		 * @return mixed
		 */
		public static function get_recovered_carts( $per_page = 20, $page_number = 1 ) {
		...
		public function no_items() {
			esc_html_e( 'No recovered cart available.', 'addify_acr' );
		}
		...
		public function prepare_items() {
			...
			$this->items = self::get_recovered_carts( $per_page, $current_page );
		...
		public function process_bulk_action() {
			...
			if ( 'delete' === $this->current_action() ) {
				...
				if ( isset( $_REQUEST['_afacr__wpnonce'] ) && ! wp_verify_nonce( esc_attr( sanitize_text_field( wp_unslash( $_REQUEST['_afacr__wpnonce'] ) ) ), '_afacr__wpnonce' ) ) {
					...
					echo '<div id="message" class="updated notice notice-success is-dismissible"><p>A cart has been deleted successfully.</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button></div>';
				}
			}
			...
			if ( ( isset( $_REQUEST['action'] ) && sanitize_text_field( wp_unslash( $_REQUEST['action'] ) ) === 'bulk-delete' ) || ( isset( $_REQUEST['action2'] ) && sanitize_text_field( wp_unslash( $_REQUEST['action2'] ) ) === 'bulk-delete' ) ) {
				...
				echo sprintf( '<div id="message" class="updated notice notice-success is-dismissible"><p>%s cart have been deleted successfully.</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button></div>', count( $delete_ids ) );
			}
		}
