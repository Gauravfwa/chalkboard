...
class Activecampaign_For_Woocommerce_Historical_Sync_Prep implements Executable, Synced_Status {
	...
	public function queue_prepared_records() {
		...
		$wpdb->query(
			...
			' SET synced_to_ac=' . self::STATUS_HISTORICAL_SYNC_PEND .
	...
	public function mark_orders_for_prep() {
		...
		$wpdb->query(
			...
			WHERE `wc_order_id` IS NOT NULL AND `synced_to_ac` NOT IN (' . $in_str . ')'
		);
		// phpcs:enable
	}
	 ...
	 */
	private function get_order_ids_by_page( $offset, $batch_limit = 100, $exclude = null, $status = null, $get_pagination = false ) {
