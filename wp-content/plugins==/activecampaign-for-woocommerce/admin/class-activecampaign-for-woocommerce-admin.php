...
class Activecampaign_For_Woocommerce_Admin implements Synced_Status {
	...
	public function enqueue_styles_scripts() {
		...
		if ( preg_match( '/^activecampaign_for_woocommerce/i', $plugin_page ) >= 1 ) {
			wp_enqueue_style(
				$this->plugin_name,
	...
	public function add_admin_page() {
		...
		$activecampaign_for_woocommerce_options = $this->get_local_settings();
		if (
		isset(
	...
	public function get_sync_ready_order_count( $type = 'int' ) {
		if ( 'array' === $type ) {
			...
			$order_totals = wc_orders_count( 'processing', 'shop_order' ) + wc_orders_count( 'completed', 'shop_order' );
		}
	 ...
	 * // TODO: This should be used! What happened to the call for this?
	 */
	...
	public function remove_deleted_order( ...$args ) {
		$logger = new Logger();
		$logger->dev( 'admin remove deleted order', [ $args ] );

		if ( isset( $args[0] ) ) {
			$order_id  = $args[0];
			$post_type = get_post_type( $order_id );
			$logger->dev( 'get_post_type', [ $post_type ] );
